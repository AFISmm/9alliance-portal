  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(0,1fr) minmax(0,1.4fr)', gap: 16, alignItems: 'start' }}>
        ...
        {/* ── Form ── */}
        <div style={{ ...CARD, padding: '20px 18px', display: 'flex', flexDirection: 'column', gap: 14 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 4 }}>
            <Send size={15} strokeWidth={1.8} style={{ color: '#C9A84C' }} />
            <span style={{ fontFamily: "'Inter', sans-serif", fontSize: 13, fontWeight: 700, color: '#F4F7FB' }}>Radicar PQR</span>
          </div>

          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
              <InputField label="Nombre"   icon={User}  value={nombre}   onChange={setNombre}   placeholder="Nombre" required />
              <InputField label="Apellido" icon={User}  value={apellido} onChange={setApellido} placeholder="Apellido" required />
            </div>
        ...
        {/* ── PQR list ── */}
        <div style={{ ...CARD, overflow: 'hidden', display: 'flex', flexDirection: 'column', maxHeight: 640 }}>
          ...
          <div style={{ overflowY: 'auto', flex: 1 }}>
            {pqrs.length === 0 ? (
              <div style={{ padding: 40, textAlign: 'center', color: '#566375', fontFamily: "'DM Sans', sans-serif", fontSize: 13 }}>
                  ...
                  <div key={p.id}>
                    ...
                    >
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontFamily: "'Inter', sans-serif", fontSize: 12.5, fontWeight: 600, color: '#F4F7FB', lineHeight: 1.2 }}>
                          {p.nombre} {p.apellido}
                        </div>
                    ...
                    {/* Expanded detail */}
                    {isSelected && (
                      <div style={{ padding: '12px 16px 14px', background: 'rgba(255,255,255,.025)', borderBottom: '1px solid rgba(255,255,255,.06)' }}>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '6px 16px', marginBottom: 10 }}>
                          {[
                            { label: 'ID', val: p.identificacion },
                            { label: 'Correo', val: p.correo },
                            { label: 'Destino', val: p.correoDestino },
                            { label: 'Fecha', val: new Date(p.timestamp).toLocaleDateString('es-CO') },
                          ].map(row => (
                            <div key={row.label}>
                        ...
                        <div style={{ marginBottom: 10 }}>
                          ...
                          <p style={{ fontFamily: "'DM Sans', sans-serif", fontSize: 12, color: '#AEBCCD', marginTop: 4, lineHeight: 1.5 }}>{p.mensaje}</p>
                        </div>
                        {/* Status change */}
                            ...
                            return (
                              <button key={s} onClick={() => changeStatus(p.id, s)}
                                style={{
                                  ...
                                  border: `1px solid ${p.estado === s ? c.color + '55' : 'rgba(255,255,255,.08)'}`,
